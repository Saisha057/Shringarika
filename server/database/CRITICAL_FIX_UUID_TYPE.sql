-- ============================================
-- CRITICAL FIX: UUID Type Mismatch in Stock Functions
-- ============================================
-- ISSUE: v_product_id was declared as TEXT but products.id is UUID
-- This caused WHERE clauses to return zero rows, making stock deduction fail
-- ============================================

-- 1. Fix: check_stock_availability
CREATE OR REPLACE FUNCTION check_stock_availability(p_items JSONB)
RETURNS JSONB AS $$
DECLARE
  v_item JSONB;
  v_product_id UUID;  -- FIXED: Changed from TEXT to UUID
  v_variant TEXT;
  v_quantity INTEGER;
...
BEGIN
  ...
  LOOP
    v_product_id := (v_item->>'productId')::UUID;  -- FIXED: Cast JSON string to UUID
...
-- SELECT proname, prosrc FROM pg_proc WHERE proname IN ('check_stock_availability', 'deduct_stock_on_order');
-- Look for "v_product_id UUID" in the source

SELECT 'CRITICAL FIX APPLIED: UUID type mismatch resolved' as status;
